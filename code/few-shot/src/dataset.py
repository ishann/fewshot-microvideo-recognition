...
class EmbeddingDataset(data.Dataset):
    ...
    def __init__(self, mode='train', root='..' + os.sep + 'dataset',
        ...
        self.root = root
        self.data_file = os.path.join(self.root, "embeddings_vid_pretrn_aud_ftune.pt")
        self.transform = transform
        self.target_transform = target_transform

        self.classes = ["Couple Romance", "Dance", "Heartbreak Romance",
                        "Memes", "Pranks"]

        data = torch.load(self.data_file)
        vids = data["vid_{}".format(mode)]
        auds = data["aud_{}".format(mode)]
        
        self.x = torch.cat([vids, auds], dim=1)
        self.y = data["{}_labels".format(mode)]
